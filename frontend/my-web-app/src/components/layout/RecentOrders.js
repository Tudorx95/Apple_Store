...
const RecentOrders = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                // For each pending order_detail, fetch the associated orders
                const allOrders = await Promise.all(
                    pendingOrders.map(async (orderDetail) => {
    ...
    return (
        <div className="my-orders-section-content">
            <h2>Recent Orders</h2>
            <p className="my-orders-section-description">Track your pending orders.</p>

            {orders.length === 0 ? (
                <div className="my-orders-empty-state">
                    <p>You have no pending orders.</p>
                    <a href="/" className="my-orders-go-to-home">Start shopping &gt;</a>
                </div>
            ) : (
